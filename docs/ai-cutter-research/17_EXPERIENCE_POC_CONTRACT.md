...
- a Good/Bad label
- a claim that a synthetic pass shows real editing preferences generalize

The vault hypothesis is **HYPOTHESIS**. A pass of sections 15 and 17 authorizes only the next experiment at the bottom of this file. It does not authorize product code.
...
The hypothesis is about this corpus and this margin rule. It is not a hypothesis about real people.
...
This is the only test that can support the hypothesis. Six copies of one waveform are still in-distribution relative to each other. They are not a sample of interviews.
...
**Unscored diagnostic `G0`, not a 13th run.** Fit a `scope=global` margin on `I01`–`I05` plus `M01`–`M06`, then evaluate only `I06`. `G0` is not part of `median_ID` or `median_OOD`. It exists because file 16 asks to see a global pool. Its number cannot produce a GO. If `G0` changes a parameter other than margin, or if its interview reduction is worse than `R06` by more than 0.10 because music-bed samples entered the median, the extractor is wrong (section 18). With the sample rule in section 9, music-bed items emit no samples, so the contract prediction is that `G0`'s margin equals `R06`'s margin.
...
If `agree` is false, that run does not support the hypothesis, even if `reduction > 0.10`.
...
Predicted activation: true on all twelve scored runs (`supportCount` is 5 on `R01`–`R06` and 6 on `R07`–`R12`; sample count is 30 or 36; `abs(400-200) = 200 > 40`). The hypothesis can still fail if those arrangements do not move the held-out costs as section 10 predicts. Activation is not success.
...
Predicted scores meet (1)–(3) if the oracles in section 10 are what the runner prints. Meeting them is evidence about this generator only.
...
## 17. Evidence required for GO

GO here means: a human may plan the next experiment in the following section. GO does not mean merge, product code, or a vault in Resonance.
...
- This sentence, verbatim, in the report: `Synthetic pass. Not evidence that human editing preferences generalize.`

Without that sentence, there is no GO.
...
| `REPORT.md` | The table in section 14 filled with measured numbers, and the verbatim sentence from section 17 when `section_15_pass` is true |
...
Run this only after section 17's GO. Do not start it from this pull request.
...
**What a pass would mean.** Only that this margin rule moved cost in the predicted direction on those files. It still would not mean a global preference, a user model, or a Resonance feature.
